...
public:
    ...
    bigint &operator%=(int x) { *this = bigint(*this % x); }
    ...
    bool get_bit(int x) const {
        if (x >= d.size() * BASE_BITS) return 0;
        return d[x / BASE_BITS] & 1 << (x % BASE_BITS);
    }

    // Sets the xth bit to v.
    void set_bit(int x, bool v) {
        if (x >= d.size() * BASE_BITS) d.resize(x / BASE_BITS + 1);
        if (v) d[x / BASE_BITS] |= 1 << (x % BASE_BITS);
        else {
            d[x / BASE_BITS] &= ~(1 << (x % BASE_BITS));
            purge();
        }
    }
    ...
    int to_int() const {
        int ret = 0;
        for (int i = (int) d.size() - 1; i >= 0; i--) ret = ret * BASE + d[i];
        return ret;
    }
...
std::string bigint::to_string(int radix) const {
    if (d.empty()) {
        return "0";
    }
    std::string ret;
...
vector <bigint> bigint::factor(bool verbose) const {
    static const int TRIVIAL_DIVISION = 0;
    static const int PRIME_SIEVE = 1000000;
    static const int POLLARD_RHO_ITERATIONS = 0;
    ...
    for (int i = 1; i < f_base.size(); i++) {
        ...
        if (start_a % 2) start_a += p;
        start_a /= 2;
        if (start_b % 2) start_b += p;
        start_b /= 2;
